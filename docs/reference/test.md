...
browser-availability check, even when media capture is disabled. The shared
agent-file capture helper allocates once per module evaluation when capture is
enabled, sharing that directory across the module's scenarios. The Node-only
