...
a prefilled issue link without starting issue creation. A definitely unstarted
CLI submission returns to the action menu, where the same failed attempt can be
chosen and confirmed again. In the Control UI, an interrupted pre-create
preparation becomes retryable after its local reservation expires. Once issue
creation starts, a result without a verified issue URL
