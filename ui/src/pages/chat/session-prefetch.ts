...
class SessionPrefetcher {
  ...
  private async prefetchEligibleSessions(): Promise<void> {
    ...
    // A presented transcript still in flight owns the socket; the committed
    // event reschedules this cycle, so waiting here costs no polling.
    if (
      !snapshot?.client ||
      !snapshot.rows ||
...
class SessionPrefetchController implements ReactiveController {
  ...
  hostConnected(): void {
    this.host.addEventListener(CHAT_TRANSCRIPT_COMMITTED_EVENT, this.sync);
  ...
  hostDisconnected(): void {
    this.host.removeEventListener(CHAT_TRANSCRIPT_COMMITTED_EVENT, this.sync);
