...
test("doctor preserves tester writability diagnostics and releases", async () => {
  ...
  assert.deepEqual(result, {
    ...
    testerGroupMembership: true,
    testerCanSendBasicMessages: false,
    sutBotGroupMembership: true,
  });
  assert.equal(proxyClosed, true);
  assert.equal(released, true);
});

test("doctor preserves unresolvable chat as structured capability health", async () => {
  const { fetchImpl, methods } = botApiFixture();
  const result = await runTelegramTestDoctor({
    acquireCredential: async () => credentialFixture(),
    fetchImpl,
  ...
  assert.equal(result.tdlibAuthorized, true);
  assert.equal(result.testerGroupMembership, false);
  assert.equal(result.testerCanSendBasicMessages, false);
  assert.equal(result.sutBotGroupMembership, true);
});

test("doctor reports inactive SUT membership as structured health", async () => {
  const { fetchImpl } = botApiFixture({ membershipStatus: "left" });
  const result = await runTelegramTestDoctor({
    acquireCredential: async () => credentialFixture(),
    fetchImpl,
    runCommandImpl: async () => statusFixture(),
  ...
  assert.equal(result.ok, false);
  assert.equal(result.testerGroupMembership, true);
  assert.equal(result.testerCanSendBasicMessages, true);
  assert.equal(result.sutBotGroupMembership, false);
});
