...
describe("Control UI assistant media e2e", () => {
  test("serves local assistant media through scoped tickets over the gateway HTTP route", async () => {
    ...
    const agentWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-media-agent-"));
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-media-outside-"));
    ...
    try {
      ...
      await Promise.all([
        fs.rm(agentWorkspace, { force: true, recursive: true }),
