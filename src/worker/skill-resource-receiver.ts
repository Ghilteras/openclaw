...
try {
 ...
 if(op.operation==='init'){
  ...
  process.stdout.write(JSON.stringify({resourceId,root,identity:identity(fs.lstatSync(name,{bigint:true}))}));
 }else {
  enter(name,op.identity);
  if(op.operation==='cleanup'){fs.rmSync(root,{recursive:true});}
  else if(op.operation==='write'){
   const parts=op.path.split('/');if(parts.some(p=>!p||p==='.'||p==='..'||/[\\\x00]/.test(p))||parts.length>${WORKER_SKILL_RESOURCE_PATH_MAX_DEPTH})throw Error('invalid resource path');
